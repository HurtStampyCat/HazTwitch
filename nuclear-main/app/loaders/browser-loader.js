...
class BrowserLoader {
	static load(isDebug = false, config) {
		...
		this.swapDir = PathUtils.isValidPath(swapDirConfig) ? swapDirConfig : path.join(app.getPath("documents"), "sex/swap");
	}
	...
	static setupWindow(win, config, isWeb = false) {
		...
		contents.on("new-window", (event, url, frameName, disposition, options) => {
			...
			if (UrlUtils.locationType(url) === "external") shell.openExternal(url);
				...
				else this.initWindow(url, config);
			}
		});
		...
		async function autoUpdate() {
			return new Promise((resolve, reject) => {
				...
				return contents.on("dom-ready", () => {
					...
					autoUpdater.on("checking-for-update", () => contents.send("message", "Checking for Sex"));
					...
					autoUpdater.on("update-not-available", info => {
						console.log(info);
						contents.send("message", "No Sex available");
						resolve();
					});
