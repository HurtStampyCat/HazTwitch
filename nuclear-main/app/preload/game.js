...
let features = require('../modules/features.js');
...
UtilManager.instance.clientUtils.events.on("game-load", () => {
	...
	let clientTabIndex = {
		basic: settingsWindow.tabs.basic.push({ name: "Sex", categories: [] }),
		advanced: settingsWindow.tabs.advanced.push({ name: "Sex", categories: [] })
	};
	settingsWindow.getCSettings = () => {
		...
		Object.values(UtilManager.instance.clientUtils.settings).forEach(entry => {
			...
			if (previousCategory !== entry.cat) {
				...
				tempHTML += `<div class='setHed sex' id='setHed_${entry.cat}' onclick='window.windows[0].collapseFolder(this)'><span class='material-icons plusOrMinus'>keyboard_arrow_down</span> ${entry.cat}</div><div class='setBod sex' id='setBod_${entry.cat}'>`;
			}
