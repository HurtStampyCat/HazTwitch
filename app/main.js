...
console.log(`SexClient@${app.getVersion()} { Electron: ${process.versions.electron}, Node: ${process.versions.node}, Chromium: ${process.versions.chrome} }`);
...
const userscriptsDir = PathUtils.isValidPath(userscriptsDirConfig) ? userscriptsDirConfig : path.join(app.getPath("documents"), "sex/scripts");
...
let init = function() {
	...
	app.once("ready", async() => {
		...
		protocol.registerFileProtocol("sex-swap", (request, callback) => callback(decodeURI(request.url.replace(/^sex-swap:/, ""))));
